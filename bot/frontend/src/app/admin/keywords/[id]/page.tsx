...
export default function EditKeywordHintPage() {
  const router = useRouter();
  const params = useParams();
  const hintId = params.id as string;
  ...
  const fetchHint = async () => {
    try {
      setLoading(true);
      const hint = await adminConfigService.getKeywordHint(hintId);
      setFormData({
        rule_name: hint.rule_name,
      ...
      toast.error(error.message || 'Không thể tải keyword hint');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await adminConfigService.updateKeywordHint(hintId, formData);
      toast.success('Đã cập nhật keyword hint');
      router.push('/admin/keywords');
    } catch (error: any) {
      toast.error(error.message || 'Không thể cập nhật keyword hint');
    } finally {
      setSaving(false);
    }
  };
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Chỉnh sửa Keyword Hint</h1>
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              Cập nhật keyword hint
            </p>
          </div>
        </div>
