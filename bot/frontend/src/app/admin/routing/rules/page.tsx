...
export default function RoutingRulesPage() {
  ...
  const fetchRules = async () => {
    try {
      setLoading(true);
      const response = await adminConfigService.listRoutingRules({ limit: 100 });
      setRules(response.items);
    } catch (error: any) {
      toast.error(error.message || 'Không thể tải routing rules');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Bạn có chắc muốn xóa routing rule này?')) return;

    try {
      await adminConfigService.deleteRoutingRule(id);
      toast.success('Đã xóa routing rule');
      fetchRules();
    } catch (error: any) {
      toast.error(error.message || 'Không thể xóa routing rule');
    }
  };
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Routing Rules</h1>
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              Quản lý intent-based routing rules
            </p>
          </div>
          <Link
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Tìm kiếm routing rules..."
            value={searchTerm}
                    ...
                    <tr key={rule.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="px-6 py-4">
                        <code className="text-xs bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">
                          {JSON.stringify(rule.intent_pattern)}
                        </code>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                        {rule.target_domain || 'N/A'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                        {rule.priority}
                      </td>
